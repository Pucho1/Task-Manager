...
const FilterTask = <T extends string>({ valueFilter, handlerFilter, options, label }: GenericFilterProps<T>) => {

    return (
        <div>
            <label className="flex sm:inline text-sm font-medium text-gray-600 mr-2 min-w-max">{label}</label>
            <select
                value={valueFilter}
